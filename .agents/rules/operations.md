# CBC Operations Rules

---

## Service Format — Absolute
...
### Per-Item Labeling Protocol
- Every item individually labeled with dietary flags (vegan, GF, nut-free, dairy-free) and consume-by times
- Zero unlabeled items leave the commissary on any order
...
- **Every item must have an individual dietary label** (vegan, GF, nut-free, dairy-free)
- **Every item must have a food safety label** stating:
  - Item name
  - Date/time of prep or packaging
  - "Consume By" time (4-hour window from when food leaves temperature control — FDA Food Code + California food safety regulations)
- **No unlabeled items leave the commissary. Zero exceptions.**
- Full dietary breakdown sheet included with every order
- **Written client acknowledgment required** before every delivery: client confirms they will communicate consume-by times to guests, discard unconsumed food after time, and not hold CBC or chef liable for food consumed after labeled time
- No confirmation = no delivery
...
**Delivery prep:**
- Depart commissary with enough time to arrive **15 minutes before** confirmed delivery window
- Text on-site contact when en route with ETA
- Count all portions against manifest before loading
...
| Window | Penalty |
|---|---|
| 7+ days before event | No fee — find replacement |
| Within 7 days | $250 fee owed to CBC |

**Chef Cancellation Response Protocol (in order):**
1. Don't tell the client yet — assess options first (2-hour target)
2. Find replacement within commissary
3. Offer cuisine substitution if needed
4. Source externally (CBC absorbs margin hit)
5. Reschedule with 10% goodwill discount
6. Full refund + 10% credit (last resort)

Rule: the client should never feel the cancellation is their problem. Doug owns resolution.

---

## Booking Confirmation Checklist

- [ ] 25% deposit collected
- [ ] Written confirmation sent (date, guest count, cuisine, delivery address, time, cancellation policy)
- [ ] Food safety acknowledgment collected from client
- [ ] Delivery details logged (address, floor/suite, parking/loading, on-site contact name + phone)
- [ ] HubSpot stage moved to **Won**
- [ ] Chef contacted to confirm availability and fulfillment terms

---

## Attended Service (Tier 2) Specifics
...
## Day-of Issue Response

| Issue | Action |
|---|---|
| Wrong item / missing portion | Call chef immediately; assess if replacement possible |
| Delivery delay | Proactively call on-site contact — never let client discover it themselves |
| Chef no-show | Activate backup chef OR contact client immediately with options |

---

## Tasting Package Rules
