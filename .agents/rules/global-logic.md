# CBC Global Logic — Immutable Project Truths

These are the non-negotiable facts about this project. Do not deviate from these without Doug's explicit instruction.

---

## Entity Structure

- **Culinary Block LLC** — the commissary (existing business, separate entity)
- **Culinary Block Collective LLC** — the catering operation (new venture, fully owned by Doug, layered on top of the commissary)
- These are two distinct legal entities. Never conflate them in copy or contracts.

---

## Tech Stack

- **Website:** Static HTML/CSS/JS — no framework, no build step
- **Hosting:** Vercel (static deployment)
- **CRM:** HubSpot (free tier)
- **Email:** Gmail (culinaryblockcatering@gmail.com)
- **AI tooling:** Claude Code, Cursor AI
- **No database.** No backend. No SaaS subscriptions beyond the above.
- Do not suggest adding a database, backend framework, or new SaaS unless Doug explicitly asks.

---

## Service Format — Non-Negotiable

- **All service is Chef's Spread (self-serve station) format only.**
- **No individual box meals. No plated service. No exceptions.**
- Minimum order: **50 guests**
- Maximum capacity: **500 guests**

---

## Pricing — Core Numbers

**Client-facing prices (what CBC charges clients):**
- Loulan · Chef Kun (Chinese/Southeast Asian): **$25/person** · 50-guest min · +$5/person 4th protein (Chinese custom) · +$1/person brown rice
- Unique Cuisine (Sushi/Japanese): **$30/person** · 50-guest min
- Five Course · Chef John — The Tortilla Theory (Mexican): **$46/person** (50–99) · **$38/person** (100+) · 50-guest min
- Five Course · Chef John — A Little Pasta Table (Italian): **$46/person** (50–99) · **$38/person** (100+) · 50-guest min
- Five Course · Chef John — Grill & Grain (Mediterranean): **$50/person** (50–99) · **$46/person** (100+) · 50-guest min
- Five Course · Chef John — Spice Route (Indian): **$40/person** · 100-guest minimum
- Attended Service (Tier 2): **+$8/person** on top of any concept price
- **No separate service or coordination fee** — CBC's margin is built into the per-guest price
- Client receives a **single per-guest line item** (plus any add-ons they choose)
- Compostable tableware (plates, cutlery, napkins) is standard on all orders. Premium compostable upgrade available at +$2.50/person.

---

## Commission — Internal Only

- Markup rates **vary by chef** and are **strictly internal**
- Never discuss, hint at, or disclose chef food prices or CBC markup in client-facing materials
- **Pricing model:** Cost-plus markup. CBC receives each chef's per-guest food price, adds markup to arrive at client price
- Chef John (Five Course): chef food prices $32 (Tortilla Theory / A Little Pasta Table) · $32 (Spice Route) · $42 (Grill & Grain). CBC sets client price independently. CBC earns spread between chef price and client price.
- Chef Kun · Loulan (楼兰): chef price $18.75 → client price $25 (CBC gets $6.25/person = ~25%)
- Unique Cuisine: chef price $24 → client price $30 (CBC gets $6/person = 20%)

---

## Regulatory Compliance

- **AB 660 (California):** Effective July 1, 2026. Requires individual dietary and food safety labeling. CBC's existing protocol already meets the standard — first-mover advantage over smaller operators who will scramble to comply.
- **Santa Clara Catering Permit:** Required before any client-facing activity. ~$282+ fee; requires commissary agreement + SOPs + inspection. **Must be filed before launch.**

---

## Chef Roster

- Roster is **dynamic** — drawn from active Culinary Block commissary tenants
- **Never state a fixed number of chefs** without a disclaimer that the roster is dynamic
- Current active chefs (as of 2026-03): Chef John (Five Course), Chef Kun · Loulan (楼兰), Unique Cuisine

---

## Sales Model

- **Phone-first.** Primary close channel is (415) 699-4397 — always visible in nav
- Phone/text response SLA: **2 hours max**
- Email/form response SLA: **24 hours max**
- Quote turnaround: **24 hours max** — this is the #1 conversion lever

---

## Brand Voice

- Professional but warm, community-oriented, chef-forward
- Say: **"independent specialist chefs"** — never "in-house chefs"
- Lead with reliability and dietary accuracy, not product features
- Service area: San Jose, Palo Alto, Mountain View, Sunnyvale, Santa Clara (within ~20 miles of San Jose commissary)

---

## Food Safety — Non-Negotiable

- Every item gets an individual dietary label (vegan, GF, nut-free, dairy-free)
- Every item gets a food safety label with a consume-by time (4-hour window, FDA Food Code)
- No unlabeled items leave the commissary — zero exceptions
- Written client acknowledgment of food safety labels required before every delivery

---

## Project Status (as of 2026-03)

- Pre-launch: infrastructure built, outreach not yet started
- No paying clients yet
- Chef agreements: Chef John drafted (needs attorney review), Chef Kun · Loulan and Unique Cuisine need formal sign-off before launch
